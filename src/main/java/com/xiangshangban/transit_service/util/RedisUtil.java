...
public class RedisUtil {
     ...
     //缓存生存时间
     private final static int EXPIRE_MIN = 15;
     //操作Key的方法 
     public Keys KEYS;
     //对存储结构为String类型的操作
     ...
     public RedisUtil() { 
      ...
      /**
       ...
       public static void init() {
    	   if(null==jedisPool){
				try {
					String host = PropertiesUtils.redisProperty("redis.host");		
					int database = Integer.parseInt(PropertiesUtils.redisProperty("redis.database"));
			   		int port = Integer.parseInt(PropertiesUtils.redisProperty("redis.port"));
		            ...
		            config.setMaxWaitMillis(Long.parseLong(PropertiesUtils.redisProperty("redis.pool.maxWaitMillis")));
		            config.setTestOnBorrow(Boolean.parseBoolean(PropertiesUtils.redisProperty("redis.pool.testOnBorrow")));
		            jedisPool = new JedisPool(config, host, port,LINK_TIMEOUT,pass, database);
				} catch (Exception e) {
        ...
        public class Hash {
            ...
            public String hget(String key, String fieid) {                
                ...
                //增加失效判断，如果失效则删除key
                long expired = 0;
                expired = sjedis.ttl(key);
                String s = null;
                if(expired>0){
                	s = sjedis.hget(key, fieid);
                }else{
                	if(sjedis.exists(key)) sjedis.del(key);
                }
                returnJedis(sjedis);
                return s;
            }
